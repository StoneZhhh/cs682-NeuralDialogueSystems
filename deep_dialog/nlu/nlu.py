...
from deep_dialog.nlu.lu_torch_net import LSTMWrapper
...
class nlu:
    ...
    def generate_dia_act(self, annot):
        ...
        if len(annot) > 0:
            tmp_annot = annot.decode().strip('.').strip('?').strip(',').strip('!')

            rep = self.parse_str_to_vector(tmp_annot)
            # Ys, cache = self.model.fwdPass(rep, self.params, predict_model=True)  # default: True
            Ys = self.model.one_batch_predict(rep)
            # Ys, loss = self.model.one_batch_train(rep)
    ...
    def load_nlu_model(self, model_path):
        ...
        if model_params['params']['model'] == 'lstm':  # lstm_
            ...
            rnnmodel = LSTMWrapper(input_size, hidden_size, output_size)
